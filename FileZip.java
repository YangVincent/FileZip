...
//			Scanner in = new Scanner(reader, "UTF-8");
			while(in.hasNext())
			{
	...
	private static boolean scanSymbol (String fileString, boolean writeToFile)
	{
		...
		for (int i = 0; i < fileString.length(); i++)
		{
			...
			if (m_dictionary.get(currentCharacter)==null)
			{
					...
					m_dictionarySize++;
					/*
					System.out.println("prev character is " + prevCharacter+" code ="+(int)prevCharacter.charAt(0));
			...
			else
			{
				...
				//System.out.println(entry + ":" + m_dictionarySize);
				m_dictionary.put(entry, m_dictionarySize);
				//System.out.println("new word A: " + entry+", code="+m_dictionarySize);
		...
		if (writeToFile)
		{
			...
			{
				//System.out.println("the character is " + m_dictionary.get(prevCharacter));
				//output.add(m_dictionary.get(prevCharacter));
    ...
    public static void unzip(String inFile, String outFile) //unzips input files
    {
    	...
    	for (int i = 0; i < inputList.length; i++)
    	{
    		...
    		zipped.add(new Integer(inputList[i]));
    	}*/
    	//List<Integer> zipped = file.split("[\\s]");
		m_dictionarySize = maxCharCode;

	//	m_unZippingDictionary = new HashMap<Integer, String>(65535);
		m_unZippingDictionary = new ArrayList<String>(65535); //starts at s^16 spots
		char[] symbol = new char[1];
		for (int i = 0; i < maxCharCode; i++)
		{
			symbol[0] = (char) i;
			//m_unZippingDictionary.put(i, "" + (char)i);
		...
		while ((i = readInt(numDictBits))!= -1)
		{
			...
			/*else if (!m_unZippingDictionary.containsKey(i)) //if the dictionary does not have the current character already
			{
				m_unZippingDictionary.put(i, "" + (char)i);
			}*/
			else
			...
			//System.out.println("dictionary size=" + m_dictionarySize);
